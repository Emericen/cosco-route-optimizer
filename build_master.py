...
ws4=wb.create_sheet('④ 运价表 Rates (MOCK)'); ws4.append(['集装箱类别','箱型','起运港','目的地','海运费USD','附加费USD','港到港USD','起运拖车USD','目的拖车USD','运输天数','运价有效期','数据来源'])
...
for r in csv.DictReader(open('ocean_edges_MOCK.csv')):
    ...
    ws4.append([CAT[box],box,A,B,oc,su,oc+su,int(door.get(A,{}).get(f'起运拖车{sz}USD',0) or 0),int(door.get(B,{}).get(f'目的拖车{sz}USD',0) or 0),int(r['运输天数']),r['运价有效期'],'MOCK'])
head(ws4,12)
